...
export async function POST() {
  try {
    ...
    const brands = await client.fetchBrands();

    return NextResponse.json({
      success: true,
      message: `Fetched ${brands.length} brands`,
      count: brands.length,
      brands: brands.slice(0, 10), // Return first 10 for preview
      sample: brands[0], // Show one full example
    });
  } catch (error) {
    return NextResponse.json(
      {
